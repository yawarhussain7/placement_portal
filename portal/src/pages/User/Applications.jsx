      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-5">
          <div className="card border rounded-lg overflow-hidden">
            <div className="p-5 border-b border-base flex flex-col lg:flex-row lg:items-center gap-3 lg:justify-between">
              <div>
                <h2 className="text-base font-bold text-primary">Application Pipeline</h2>
                <p className="text-xs text-muted mt-1">Select any application to edit its live placement details.</p>
              </div>
              <div className="relative w-full lg:w-72">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" />
                <input value={query} onChange={(event) => setQuery(event.target.value)} className="input-base w-full border rounded-lg pl-9 pr-3 py-2 text-sm" placeholder="Search applications" />
              </div>
            </div>

            <div className="divide-y divide-base">
              {filtered.map((app) => (
                <button key={app.id} onClick={() => setSelectedId(app.id)} className={`w-full text-left p-5 hover:bg-subtle ${selected?.id === app.id ? 'bg-accent-subtle' : ''}`}>
                  <div className="flex flex-col xl:flex-row xl:items-center gap-4 xl:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="font-bold text-primary">{app.role}</h3>
                        <span className={`text-[11px] font-bold px-2 py-1 rounded-full ${statusTone[app.status] || 'bg-subtle text-secondary'}`}>{app.status}</span>
                      </div>
                      <p className="text-sm text-secondary mt-1">{app.company}</p>
          ...
          <aside className="space-y-5">
            {selected && (
              <form className="card border rounded-lg p-5 space-y-4" onSubmit={(event) => event.preventDefault()}>
                <div>
                  <h2 className="font-bold text-primary">Edit Application</h2>
                  <p className="text-xs text-muted mt-1">{selected.id}</p>
                </div>
                <label className="space-y-2 block">
                  <span className="text-xs font-bold text-primary">Role</span>
                  <input className="input-base w-full border rounded-lg px-3 py-2 text-sm" value={selected.role} onChange={(event) => updateApplication(selected.id, { role: event.target.value })} />
                </label>
                <label className="space-y-2 block">
                  <span className="text-xs font-bold text-primary">Status</span>
                  <select className="input-base w-full border rounded-lg px-3 py-2 text-sm" value={selected.status} onChange={(event) => updateApplication(selected.id, { status: event.target.value })}>
                    <option>Draft</option>
                ...
                <label className="space-y-2 block">
                  ...
                  <textarea className="input-base w-full border rounded-lg px-3 py-2 text-sm resize-none" rows={4} value={selected.notes} onChange={(event) => updateApplication(selected.id, { notes: event.target.value })} />
                </label>
              </form>
            )}

            <form onSubmit={createApplication} className="card border rounded-lg p-5 space-y-4">
              <div>
                <h2 className="font-bold text-primary">Create Draft</h2>
                <p className="text-xs text-muted mt-1">Add a placement target to the pipeline.</p>
              </div>
              <input className="input-base w-full border rounded-lg px-3 py-2 text-sm" placeholder="Role title" value={draft.role} onChange={(event) => setDraft((current) => ({ ...current, role: event.target.value }))} />
              <input className="input-base w-full border rounded-lg px-3 py-2 text-sm" placeholder="Company name" value={draft.company} onChange={(event) => setDraft((current) => ({ ...current, company: event.target.value }))} />
              <button className="w-full inline-flex items-center justify-center gap-2 bg-accent hover-accent text-white px-4 py-2 rounded-lg text-sm font-bold">
                <Plus size={16} />
                Add Draft
              </button>
            </form>
